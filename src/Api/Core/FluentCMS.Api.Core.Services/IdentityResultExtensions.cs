...
public static class IdentityResultExtensions
{
    ...
    /// <exception cref="EnhancedException">Thrown when the IdentityResult indicates validation errors.</exception>
    public static void ThrowIfInvalid(this IdentityResult identityResult)
    {
        if (!identityResult.Succeeded)
        {
            throw new EnhancedException(identityResult.Errors.Select(e => $"User.{e.Code}"));
        }
    }
}
