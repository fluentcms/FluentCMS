...
public class PluginRepository(CmsCoreDbContext dbContext) : SiteAssociatedRepository<Plugin>(dbContext), IPluginRepository
{
    public async Task<IEnumerable<Plugin>> GetByPageId(Guid pageId, CancellationToken cancellationToken = default)
    {
        return await Query().Where(x => x.PageId == pageId).ToList(cancellationToken);
    }
