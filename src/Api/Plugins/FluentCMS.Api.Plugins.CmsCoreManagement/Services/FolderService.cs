...
internal class FolderService(IFolderRepository folderRepository, IFileRepository fileRepository) : IFolderService
{
    ...
    public async Task<Folder> Create(Folder folder, CancellationToken cancellationToken = default)
    {
        ...
        if (!IsValidFolderName(folder.NormalizedName))
            throw new EnhancedException(MessageCodes.FolderInvalidName);
    ...
    {
        return await folderRepository.GetByFolderId(folderId, cancellationToken);
    }

    public async Task<Folder> Remove(Guid id, CancellationToken cancellationToken = default)
    {
        // TODO: Delete sub folders and files
        var existingFolder = await folderRepository.GetById(id, cancellationToken) ??
            throw new EnhancedException(MessageCodes.FolderNotFound);

        return await folderRepository.Remove(id, cancellationToken);
    }
    ...
    public async Task<Folder> Rename(Guid folderId, string name, CancellationToken cancellationToken = default)
    {
        ...
        if (existingFolder.ParentId == null)
            throw new EnhancedException(MessageCodes.FolderCannotRenameRootFolder);
        ...
        return await folderRepository.Update(existingFolder, cancellationToken) ??
            throw new EnhancedException(MessageCodes.FolderUnableToUpdate);
    }
    ...
    public async Task<Folder> Move(Guid folderId, Guid parentFolderId, CancellationToken cancellationToken = default)
    {
        ...
        if (folderId == parentFolderId)
            throw new EnhancedException(MessageCodes.FolderCannotMoveToItself);
        ...
        // check if the folder is not root folder
        if (existingFolder.ParentId == null)
            throw new EnhancedException(MessageCodes.FolderCannotMoveRootFolder);
        ...
        if (parentFolders.Any(x => x.Id == folderId))
            throw new EnhancedException(MessageCodes.FolderCannotMoveToChild);
