...
public class PluginRepository(PostgresDbContext context) : SiteAssociatedRepository<Plugin>(context), IPluginRepository, IService
{
    public async Task<IEnumerable<Plugin>> GetByPageId(Guid pageId, CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();

        return await Table.Where(x => x.PageId == pageId).ToListAsync(cancellationToken);
    }
