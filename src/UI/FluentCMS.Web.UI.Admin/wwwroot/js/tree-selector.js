export function TreeSelector(
    el,
    { cssPrefix = "tree", options = [], onChange, value } = {}
) {
    const menu = document.createElement("div");
    const input = document.createElement("input");
    input.setAttribute(
        "placeholder",
        el.getAttribute("placeholder") ?? "Choose an item"
    );
    ...
    function setValue(value) {
        ...
        el.querySelectorAll("." + cssPrefix + "-option-active").forEach((el) =>
            el.classList.remove(cssPrefix + "-option-active")
        );
        if (!value) {
            input.value = "";
            ...
            el.querySelector(`[data-value="${value}"]`).classList.add(
                cssPrefix + "-option-active"
            );
        }
        closeMenu();
    }
    ...
    function toggleCollapsible(optionEl) {
        optionEl
            .querySelector("." + cssPrefix + "-menu-inner")
            .classList.toggle(cssPrefix + "-menu-inner-open");
    }
        ...
        for (let option of options) {
            ...
            setTimeout(() => {
                if (option.items?.length) {
                    const menuEl = optionEl.querySelector(
                        "." + cssPrefix + "-menu-inner"
                    );
                    setInnerOptions(option.items, menuEl);
                }
                if (option.items?.length) {
                    optionEl
                        .querySelector(`.${cssPrefix}-option-toggler`)
                ...
                optionEl
                    .querySelector(`.${cssPrefix}-option-content`)
                    .addEventListener("click", () => {
