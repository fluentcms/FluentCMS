    ...
    where TDto : class
{
    ...
    public GenericCrudServiceWithMapping(IServiceProvider serviceProvider) : base(serviceProvider)
    {
        ...
        _validator = serviceProvider.GetRequiredService<AbstractValidator<TDto>>();
    }
    ...
    public virtual async Task CreateAsync(TDto dto)
    {
        ...
        await base.CreateAsync(await MapToEntity(dto));
    }

    private async Task ValidateDto(TDto dto)
    {
    ...
    {
        await ValidateDto(dto);
        await base.UpdateAsync(await MapToEntity(dto));
    }

    public virtual async Task UpdateAsync(Guid id, JsonPatchDocument<TDto> dto)
    {
        // todo handle Validation
        await base.UpdateAsync(await MapToEntity(id, dto));
    }
    ...
    where TListDto : class
{
    ...
    private readonly AbstractValidator<TEditDto> _editDtoValidator;
    ...
    public GenericCrudServiceWithMapping(IServiceProvider serviceProvider) : base(serviceProvider)
    {
        ...
        _createDtoValidator = serviceProvider.GetRequiredService<AbstractValidator<TCreateDto>>();
        _editDtoValidator = serviceProvider.GetRequiredService<AbstractValidator<TEditDto>>();
    }
    ...
    private async Task ValidateDto(TCreateDto dto)
    {
    ...
    {
        await ValidateDto(dto);
        await base.CreateAsync(await MapToFromCreateDto(dto));
    }
    private async Task ValidateDto(TEditDto dto)
    {
        var result = await _editDtoValidator.ValidateAsync(dto);
        if (!result.IsValid)
        {
            throw new FluentValidation.ValidationException(result.Errors);
        }
    ...
    {
        await ValidateDto(dto);
        await base.UpdateAsync(await MapToFromEditDto(dto));
    }
