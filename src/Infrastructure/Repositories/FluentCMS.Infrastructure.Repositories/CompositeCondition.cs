...
public class CompositeCondition(bool useAndLogic, params IDbInitializationCondition[] conditions) : IDbInitializationCondition
{
    // Store conditions in a list for easier manipulation and enumeration
    private readonly List<IDbInitializationCondition> _conditions = conditions?.ToList() ?? throw new ArgumentNullException(nameof(conditions));
